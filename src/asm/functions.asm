...
;Эта функция вычисляет 2^x + 1:

;Стандартный пролог функции
;Загрузка аргумента x в стек FPU
;Разделение x на целую и дробную части
;Вычисление 2^x с помощью f2xm1 (2^x-1) для дробной части
;Умножение на 2^(целая часть) с помощью fscale
;Добавление 1 к результату
;Возврат результата через ST0

;Эта функция вычисляет x^5:

;Загружает x
;Вычисляет x^2 путем умножения x на себя
;Вычисляет x^4 путем умножения x^2 на себя
;Умножает x^4 на x, получая итоговый результат x^5

;f3: Эта функция вычисляет (1-x)/3:

;Загружает x
;Вычисляет 1-x используя обратное вычитание (fsubr)
;Делит результат на 3